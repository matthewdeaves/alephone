#!/usr/bin/env bash
# build-deps-i386.sh - cross-compile the i386 slice's dependencies (alephone#30)
# Target: i686-apple-darwin8, sysroot: MacOSX10.4u.sdk, floor 10.4
# Toolchain: ~/oldmac/gcc14-i686 on mini-intel2 (old-mac-build-host#85); the
#            only host that has it, so this pins the build-host picker to it.
# Output prefix: /Users/mini/oldmac/alephone/i386-deps
#
...
echo "[deps-i386] running cross-compilation on $BUILD_HOST..."
ssh "$BUILD_HOST" 'bash -s' << 'REMOTE_SCRIPT'
set -euo pipefail

SRC=~/oldmac/alephone/deps-src
BUILD=~/oldmac/alephone/deps-build-i386
PREFIX=~/oldmac/alephone/i386-deps
...
COMMON_LDFLAGS="-mmacosx-version-min=10.4 -isysroot $SDK -L$PREFIX/lib"
...
if [ ! -f "$PREFIX/lib/libsndfile.a" ]; then
    ...
    # Patch missing strnlen (not in libSystem before 10.7)
...
if [ ! -f "$PREFIX/lib/libopenal.a" ]; then
    ...
    # Generate config.h (same as the PPC recipe: no cmake on mini-intel2)
    cat > config.h << 'EOFCFG'
#pragma once
...
prefix=/Users/mini/oldmac/alephone/i386-deps
exec_prefix=${prefix}
...
echo "[deps-i386] all i386 dependencies built and verified successfully in $PREFIX"
ls -la "$PREFIX/lib"
REMOTE_SCRIPT
