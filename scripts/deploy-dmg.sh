...
#         plus its sibling data files) to /Applications, strips quarantine
...
REMOTE_ROOT_REL="oldmac/alephone/deploy"
REMOTE_DMG_NAME="$DMG_BASENAME"
REMOTE_MOUNT_NAME="mount.$$"
REMOTE_QCLEAR_NAME="clear-launch-quarantine.sh"
QCLEAR_SRC="$REPO_ROOT/scripts/clear-launch-quarantine.sh"

ssh "$HOST" 'mkdir -p "$HOME/oldmac/alephone/deploy"'
echo "[deploy] copying $DMG_BASENAME to $HOST:$REMOTE_ROOT_REL/$REMOTE_DMG_NAME..."
scp -q "$DMG_PATH" "$HOST:$REMOTE_ROOT_REL/$REMOTE_DMG_NAME"
...
[ -f "$QCLEAR_SRC" ] && scp -q "$QCLEAR_SRC" "$HOST:$REMOTE_ROOT_REL/$REMOTE_QCLEAR_NAME"

echo "[deploy] mounting and installing on $HOST..."
ssh "$HOST" bash -s -- "$REMOTE_DMG_NAME" "$REMOTE_MOUNT_NAME" "$REMOTE_QCLEAR_NAME" << 'REMOTE_DEPLOY'
...
set -eu
DEPLOY_ROOT="$HOME/oldmac/alephone/deploy"
DMG="$DEPLOY_ROOT/$1"
MOUNT="$DEPLOY_ROOT/$2"
QCLEAR="$DEPLOY_ROOT/$3"
mkdir -p "$MOUNT"
...
# shell array: some fleet targets run an ancient /bin/bash).  Keep this
# transfer record under oldmac and scope quarantine clearing to apps from this
# deploy, never every app in /Applications.
DEPLOYED_LIST="$DEPLOY_ROOT/deployed-apps.$$"
: > "$DEPLOYED_LIST"
...
ssh "$HOST" "rm -f '$REMOTE_ROOT_REL/$REMOTE_DMG_NAME' '$REMOTE_ROOT_REL/$REMOTE_QCLEAR_NAME'"
echo "[deploy] done: apps are installed under /Applications on $HOST"
